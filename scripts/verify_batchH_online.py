...
print('== DD-HG 段清点(批次H,期望78) ==')
rows = query({'select': 'slug,name_zh,sku,brand,status',
              'sku': 'ilike.DD-HG*', 'order': 'sku', 'limit': '200'})
...
print('== 康迈轮戩(期望2) ==')
rows2 = query({'select': 'slug,name_zh,brand,status', 'brand': 'eq.康迈'})
if rows2 and 'error' in rows2[0]:
    print('FAIL', rows2[0]['error'][:80])
else:
    for p in rows2:
        print('  ', p.get('slug'), '|', p.get('name_zh'), '|', p.get('status'))
    print(f'合计: {len(rows2)}')
...
print('== DB 总数(期望 11835) ==')
print('  total =', count_all())
print('== 线上API抽查 ==')
try:
    u = 'https://products.dedaautoparts.com/api/search?' + urllib.parse.urlencode({'q': '华盖', 'limit': '3'})
