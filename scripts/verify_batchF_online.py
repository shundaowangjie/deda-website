...
print('== DD-RL 段清点(批次F,期望1776) ==')
rows = query({'select': 'slug,name_zh,sku,brand,status',
              'sku': 'ilike.DD-RL*', 'order': 'sku', 'limit': '2000'})
...
print('== DB 总数(期望 11173) ==')
print('  total =', count_all())
print('== 线上API抽查 ==')
try:
    u = 'https://products.dedaautoparts.com/api/search?' + urllib.parse.urlencode({'q': '瑞立', 'limit': '3'})
