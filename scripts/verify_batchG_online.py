...
print('== DD-FK 段清点(批次G,期望582) ==')
rows = query({'select': 'slug,name_zh,sku,brand,status',
              'sku': 'ilike.DD-FK*', 'order': 'sku', 'limit': '800'})
...
print('== DB 总数(期望 11755) ==')
print('  total =', count_all())
print('== 线上API抽查 ==')
try:
    u = 'https://products.dedaautoparts.com/api/search?' + urllib.parse.urlencode({'q': '隆顺达', 'limit': '3'})
