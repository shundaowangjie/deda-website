...
print('== DD-ZP 段清点(批次E,期望680) ==')
rows = query({'select': 'slug,name_zh,sku,brand,status',
              'sku': 'ilike.DD-ZP*', 'order': 'sku', 'limit': '800'})
...
print('== DD-RL 探测(批次F,未执行则0) ==')
rows2 = query({'select': 'sku', 'sku': 'ilike.DD-RL*', 'limit': '2000'})
print('  DD-RL 已入库:', len(rows2) if not (rows2 and 'error' in rows2[0]) else rows2[0]['error'][:60])
...
print('== 线上API抽查 ==')
try:
    u = 'https://products.dedaautoparts.com/api/search?' + urllib.parse.urlencode({'q': '中配', 'limit': '3'})
